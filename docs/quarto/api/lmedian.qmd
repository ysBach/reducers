# lmedian { #reducers.lmedian }

```python
lmedian(a, axis=None, *, ignore_inf=False, validate=True)
```
...
<code>[**out**]{.parameter-name} [:]{.parameter-annotation-sep} [scalar or ndarray]{.parameter-annotation}</code>
