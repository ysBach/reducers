# count_finite { #reducers.count_finite }

```python
count_finite(a, axis=None, *, validate=True)
```
...
<code>[**out**]{.parameter-name} [:]{.parameter-annotation-sep} [int or ndarray of int]{.parameter-annotation}</code>

:   Number of finite values. ``axis=None`` returns a Python ``int``. Axis reductions return an integer array with the reduced axis removed.
